...
class OpenID_Connect_Generic_Client_Wrapper {
	...
	function authentication_request_callback() {
		...
		if ( ! $user ) {
			...
			//$this->update_user_with_claims($user,$user_claim);
		}
		else {
			//$this->update_user_with_claims($user,$user_claim);
	...
	function update_user_with_claims($user, $user_claims){
		$update_userdata = array();
		$update_userdata['ID'] = $user->ID;

		if($user->user_login == $user->display_name && !empty($user_claims['name'])){
			$update_userdata['display_name'] = $user_claims['name'];
		}

		if(empty($user->first_name) && !empty($user_claims['given_name'])){
			$update_userdata['first_name'] = $user_claims['given_name'];
		}

		if(empty($user->last_name) && !empty($user_claims['family_name'])){
			$update_userdata['last_name'] = $user_claims['family_name'];
		}
